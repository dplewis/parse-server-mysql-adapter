...
const mysqlURI = 'mysql://parse:parsetest@localhost:3306/parse_server_mysql_adapter_test_database';
const mariaURI = 'mysql://root@localhost:3306/parse_server_maria_adapter_test_database';
...
if (process.env.PARSE_SERVER_TEST_DB === 'postgres') {
  ...
  databaseURI = process.env.PARSE_SERVER_TEST_DATABASE_URI || mysqlURI;
  databaseAdapter = new MySQLStorageAdapter({
    uri: databaseURI,
    collectionPrefix: 'test_',
  });
} else if (process.env.PARSE_SERVER_TEST_DB === 'maria') {
  databaseURI = process.env.PARSE_SERVER_TEST_DATABASE_URI || mariaURI;
  databaseAdapter = new MySQLStorageAdapter({
    uri: databaseURI,
    collectionPrefix: 'test_',
  });
...
const skippedTest = require('./skippedTests.json');
const _it = it;
global.it = (expectations, assertion, timeout) => {
  if (skippedTest.includes(expectations)) {
    return xit(expectations, assertion, timeout);
  }
  return _it(expectations, assertion, timeout);
};
